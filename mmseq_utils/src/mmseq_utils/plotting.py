...
from mmseq_utils.casadi_struct import (
    casadi_sym_struct,
    reconstruct_sym_struct_map_from_array,
)
from mmseq_utils.math import wrap_pi_array

from mmseq_control.robot import CasadiModelInterface, MobileManipulator3D
from mmseq_control_new.MPCConstraints import (
    SignedDistanceConstraint,
...
class DataPlotter:
    ...
    def from_ROSEXP_results(cls, folder_path, process):
        ...
        xs = np.hstack(
            (
                ros_data["ridgeback"]["joint_states"]["qs"],
        ...
        ts = (
            [ros_data["ridgeback"]["joint_states"]["ts"]]
            + [ros_data["model"]["EE"]["ts"]] * 4
            + [ros_data["model"]["base"]["ts"]] * 4
        )
        keys = [
            "xs",
            "r_ew_ws",
    ...
    def run_mpc_iter(self, t):
        ...
        iter_snapshot = self._convert_np_array_to_dict(
            self.data["mpc_iter_snapshots"][t_index]
        )
    ...
    def plot_ee_tracking(self, axes=None, index=0, legend=None, base_frame=False):
        ...
        if base_frame:
            ...
            r_ew_ws = [
                self._transform_w2b_SE3(self.data["xs"][k, :3], self.data["r_ew_ws"][k])
                for k in range(N)
            ]
    ...
    def plot_base_path(
        self, axes=None, index=0, legend=None, worldframe=True, linewidth=1, color=None
    ):
        if worldframe:
        ...
        if len(r_b) > 0:
            axes.plot(
                r_b[:, 0], r_b[:, 1], label=legend, color=color, linewidth=linewidth
            )
    ...
    def plot_base_ref_path(
        self, axes=None, index=0, legend=None, worldframe=True, color="b"
    ):
        if worldframe:
    ...
    def plot_base_tracking(self, axes=None, index=0, legend=None):
        ...
        if len(yaw_bw_w_ds) > 0:
            axes.plot(
                ts, yaw_bw_w_ds, label=legend + "$Θ_d$", color="b", linestyle="--"
            )
        if len(yaw_bw_ws) > 0:
    ...
    def plot_base_state_separate(
        self, axes=None, index=0, legend=None, linewidth=1, color=None
    ):
        ts = self.data["ts"]
        r_ew_ws = self.data.get("r_bw_ws", [])
        yaw_bw_ws = self.data.get("yaw_bw_ws", [])
    ...
    def plot_ee_orientation_tracking(self, axes=None, index=0, legend=None):
        ...
        axes.plot(
            ts, Q_we_ds[:, 0], label=legend + "$Q_{d,x}$", color="r", linestyle="--"
        )
        axes.plot(
            ts, Q_we_ds[:, 1], label=legend + "$Q_{d,y}$", color="g", linestyle="--"
        )
        axes.plot(
            ts, Q_we_ds[:, 2], label=legend + "$Q_{d,z}$", color="b", linestyle="--"
        )
    ...
    def plot_ee_linear_velocity_tracking(self, axes=None, index=0, legend=None):
        ...
        if len(v_ref) > 0:
            axes.plot(
                ts, v_ref[:, 0], label=legend + "${v_x}_d}$", color="r", linestyle="--"
            )
            axes.plot(
                ts, v_ref[:, 1], label=legend + "${v_y}_d}$", color="g", linestyle="--"
            )
            axes.plot(
                ts, v_ref[:, 2], label=legend + "${v_z}_d}$", color="b", linestyle="--"
            )
    ...
    def plot_tracking_err(self, axes=None, index=0, legend=None):
        ...
        axes.plot(
            ...
            label=legend
            + " $err_{base}, rms_{base} = $"
            + str(self.data["statistics"]["err_base"]["rms"]),
            linestyle="--",
            color=colors[index],
        )
        axes.plot(
            ...
            label=legend
            + " $err_{ee}, rms_{ee} = $"
            + str(self.data["statistics"]["err_ee"]["rms"]),
            linestyle="-",
            color=colors[index],
        )
    ...
    def plot_collision(self):
        ...
        for name, sd_per_pair in sds.items():
            ...
            margin = self.config["controller"]["collision_safety_margin"].get(
                name, None
            )
            if margin is None:
                margin = self.config["controller"]["collision_safety_margin"].get(
    ...
    def plot_sdf_collision_separate(
        self, axes=None, index=0, color=None, linewidth=1, block=True, legend=None
    ):
        nq = int(self.data["nq"])
        ts = self.data["ts"]
        qs = self.data["xs"][:, :nq]
    ...
    def plot_collision_detailed(self):
        ...
        n_figs = (
            n_total + data_per_fig - 1
        ) // data_per_fig  # Number of figures needed
    ...
    def plot_cost_htmpc(self, axes=None, index=0, block=True, legend=None):
        ...
        if cost is not None:
            ...
            for l in range(task_num):
                ...
                ax[l].plot(
                    xticks + HT_iter_num * ST_iter_num - 1,
                    cost_final[:, l],
                    "x-",
                    label=legend + " final",
                    linewidth=2,
    ...
    def plot_solver_status_htmpc(self, axes=None, index=0, block=True, legend=None):
        axes = self.plot_time_series_data_htmpc(
            "mpc_solver_statuss", axes, index, False, legend
        )
    ...
    def plot_solver_iters_htmpc(self, axes=None, index=0, block=True, legend=None):
        axes = self.plot_time_series_data_htmpc(
            "mpc_qp_iters", axes, index, False, legend
        )
    ...
    def plot_time_htmpc(self, axes=None, index=0, block=True, legend=None):
        for data_name in self.data.keys():
            if "mpc_time" in data_name:
                axes = self.plot_time_series_data_htmpc(
                    data_name, axes, index, False, legend
                )

        return axes

    def plot_time_series_data_htmpc(
        self, data_name, axes=None, index=0, block=True, legend=None
    ):
        ...
        if HT_iter_num == task_num == ST_iter_num == 1:
            axes[0].plot(
                ...
                data.squeeze(),
                ".-",
                label=" ".join([legend] + data_name.split("_")[1:]),
                linewidth=2,
                markersize=8,
            )
            axes[0].grid("on")
    ...
    def plot_task_performance(self, axes=None, index=0, legend=None):
        if axes is None:
            ...
            if len(axes) != 4:
                raise ValueError(
                    "Given axes number ({}) does not match task number ({}).".format(
                        len(axes), 4
                    )
                )
        ...
        t_sim = self.data["ts"]
        nq = int(self.data["nq"])
        xs_sat, us_sat = self.model_interface.robot.checkBounds(
            self.data["xs"], self.data["cmd_accs"], -1e-3
        )
        ...
        axes[0].plot(
            ...
            label=legend
            + " mean = {:.3f}".format(
                self.data["statistics"]["constraints_violation"]["mean"] * 100
            ),
            color=colors[index],
        )
        ...
        axes[1].plot(
            ...
            self.data["err_ee"],
            label=legend
            + " acc = {:.3f}".format(self.data["statistics"]["err_ee"]["integral"]),
            color=colors[index],
        )
        ...
        axes[2].plot(
            ...
            self.data["err_base"],
            label=legend
            + " acc = {:.3f}".format(self.data["statistics"]["err_base"]["integral"]),
            color=colors[index],
        )
    ...
    def plot_task_violation(self, axes=None, index=0, legend=None):
        ...
        for tid in range(task_num):
            ...
            if False:
                ...
                axes[tid].set_ylabel(
                    "# Inequalities Saturated \n out of {}".format(w.shape[1])
                )
            else:
                axes[tid].plot(
                    t_sim, np.linalg.norm(w, axis=1), color=colors[index], label=legend
                )

            axes[tid].set_title("Task " + str(tid) + " " + task_names[tid])

        axes[-1].plot(
            t_sim, self.data["arm_manipulability"], color=colors[index], label=legend
        )
        ...
        plt.plot(
            t_sim, np.linalg.norm(w0[:, 18:36], axis=1), label=legend + "_joint_angle"
        )
        plt.plot(
            t_sim, np.linalg.norm(w0[:, 36:54], axis=1), label=legend + "_joint_acc"
        )
    ...
    def plot_mpc_prediction(
        self, data_name, t=0, axes=None, index=0, block=True, legend=None, p_struct=None
    ):
        t_sim = self.data["ts"]
        t_index = np.argmin(np.abs(t_sim - t))
        off_set = t_index % 4
            ...
            for constraint in self.controller.constraints:
                ...
                for t_index in range(N):
                    if p_struct is None:
                        param_map_bar = [
                            reconstruct_sym_struct_map_from_array(
                                self.controller.p_struct, param_map_array
                            )
                            for param_map_array in self.data["mpc_ocp_params"][t_index]
                        ]
                    else:
                        param_map_bar = [
                            reconstruct_sym_struct_map_from_array(
                                p_struct, param_map_array
                            )
                            for param_map_array in self.data["mpc_ocp_params"][t_index]
                        ]
                    ...
                    self.data[data_name].append(
                        self.controller.evaluate_constraints(
                            constraint, x_bar, u_bar, param_map_bar
                        )
                    )
        ...
        for fi in range(figs_num):
            if axes is None or fi != 0:
                data_num = (
                    data_per_figure if fi < figs_num - 1 else D - fi * data_per_figure
                )
            ...
            for i in range(data_num):
                ...
                axes[i].plot(
                    t_all[:, 0].flatten(),
                    data[:, 0, data_index].flatten(),
                    "o-",
                    label=" ".join(["actual", legend]),
                    linewidth=2,
                    markersize=8,
                )
                axes[i].plot(
                    t_all[off_set::5].T,
                    data[off_set::5, :, data_index].T,
                    "o-",
                    linewidth=1,
                    fillstyle="none",
                )
    ...
    def plot_mpc_prediction_cbf_gamma(
        self, data_name, t=0, axes=None, index=0, block=True, legend=None
    ):
        t_sim = self.data["ts"]
        t_index = np.argmin(np.abs(t_sim - t))
        off_set = t_index % 4
        ...
        for constraint in self.controller.constraints:
            ...
            for t_index in range(N):
                ...
                u_bar = self.data["mpc_u_bars"][t_index]
                self.data[data_name].append(
                    self.controller.evaluate_cbf_gammas(
                        constraint, x_bar, u_bar, param_map_bar
                    )
                )
            self.data[data_name] = np.array(self.data[data_name])
    ...
    def save_figs(self):
        ...
        multipage(
            Path(str(self.data["folder_path"])) / Path(folder_name) / "report.pdf", figs
        )
    ...
    def plot_exp(self, time):
        ...
        self.plot_mpc_prediction_cbf_gamma(
            "mpc_sdf_constraint_predictions", t=0, block=False
        )
    ...
    def plot_sdf(self, t, use_iter_snapshot=False, block=True):
        ...
        if use_iter_snapshot:
            iter_snapshot = dict(
                enumerate(self.data["mpc_iter_snapshots"][t_index].flatten(), 1)
            )[1]
            param_map_array = iter_snapshot["p_map_bar"][0]
            param_map = reconstruct_sym_struct_map_from_array(
                self.controller.p_struct, param_map_array
            )
        ...
        sdf_map.update_map(
            param_map["x_grid_sdf"].toarray().flatten(),
        ...
        const = SignedDistanceConstraintCBF(
            self.model_interface.robot,
            self.model_interface.getSignedDistanceSymMdls("sdf"),
            self.config["controller"]["collision_safety_margin"]["sdf"],
            "sdf",
        )
        ...
        ax1.plot(
            ee_pos[collision_free, 0],
            ee_pos[collision_free, 1],
            ee_pos[collision_free, 2],
            color="g",
        )
        ax1.plot(
            ee_pos[collided, 0], ee_pos[collided, 1], ee_pos[collided, 2], color="r"
        )
        ...
        y_lim = [-1, 2]
        z_lim = [0.18, 0.43]
        sdf_map.vis(x_lim=x_lim, y_lim=y_lim, z_lim=[0.1, 0.1], block=False)
        sdf_map.vis(x_lim=x_lim, y_lim=y_lim, z_lim=[0.2, 0.2], block=False)
        sdf_map.vis(x_lim=x_lim, y_lim=y_lim, z_lim=[0.3, 0.3], block=False)
        sdf_map.vis(x_lim=x_lim, y_lim=y_lim, z_lim=[0.4, 0.4], block=False)
    ...
    def plot_sdf_map(self, t, use_iter_snapshot=False, block=True, p_struct=None):
        ...
        for z in z_lims:
            sdf_map.vis(x_lim=x_lim, y_lim=y_lim, z_lim=[z, z], block=False)
    ...
    def check_sdf(self):
        ...
        for i in range(N - 1):
            ...
            param_map_next = reconstruct_sym_struct_map_from_array(
                self.controller.p_struct, param_map_array_next
            )
            keys = ["x_grid_sdf", "y_grid_sdf", "z_grid_sdf", "value_sdf"]
            for key in keys:
                diff = np.linalg.norm(
                    param_map[key].toarray().flatten()
                    - param_map_next[key].toarray().flatten()
                )
    ...
    def sdf_failure_debug(self, t):
        ...
        self.plot_time_series_data_htmpc(
            "mpc_solver_statuss", axes=[axes[1]], block=False
        )
    ...
    def print_mpc_param(self, param_name, p_struct=None):
        if p_struct is None:
            param_map_bar = [
                reconstruct_sym_struct_map_from_array(
                    self.controller.p_struct, param_map_array[0]
                )
                for param_map_array in self.data["mpc_ocp_params"]
            ]
        else:
            param_map_bar = [
                reconstruct_sym_struct_map_from_array(p_struct, param_map_array[0])
                for param_map_array in self.data["mpc_ocp_params"]
            ]
...
class ROSBagPlotter:
    def __init__(
        self, bag_file, config_file="$(rospack find mmseq_run)/config/robot/thing.yaml"
    ):
    ...
    def parse_base_tf_msgs(self, bag):
        ...
        ts, p_bws, orn_bws = ros_utils.parse_tf_messages(
            tf_msgs, "base_link", "my_world"
        )
    ...
    def parse_joint_states(self, bag):
        ...
        ridgeback_msgs = [
            msg for _, msg, _ in bag.read_messages("/ridgeback/joint_states")
        ]
        ...
        tbs, qbs, vbs = ros_utils.parse_ridgeback_joint_state_msgs(
            ridgeback_msgs, False
        )
        self.data["ur10"]["joint_states"] = {
            "ts": tas,
            "qs": qas,
            "vs": vas,
            "effs": effas,
        }  # 125hz
        self.data["ridgeback"]["joint_states"] = {
            "ts": tbs,
            "qs": qbs,
            "vs": vbs,
        }  # 50hz
        ...
        if qas.shape[0] != 0:
            ...
            self.data["ur10"]["joint_states_interpolated"] = {
                "ts": tbs,
                "qs": qas_interp,
                "vs": vas_interp,
            }
    ...
    def parse_cmd_vels(self, bag):
        ...
        # ridgeback
        ridgeback_ts = np.array(
            [t.to_sec() for _, _, t in bag.read_messages("/ridgeback/cmd_vel")]
        )
        ridgeback_cmd_msgs = [
            msg for _, msg, _ in bag.read_messages("/ridgeback/cmd_vel")
        ]
        ...
        self.data["ridgeback"]["cmd_vels"] = {
            "ts": ridgeback_ts,
            "vc_bs": ridgeback_cmd_vels,
        }

    def parse_mpc_tracking_pt(self, bag):
        tracking_pt_msgs = [msg for _, msg, _ in bag.read_messages("/mpc_tracking_pt")]
        t_rs, pose_rs = ros_utils.parse_multidofjointtrajectory_msg(
            tracking_pt_msgs, False
        )
        self.data["mpc"]["tracking_pt"] = {
            "ts": t_rs,
            "rees": pose_rs.get("EE", []),
    ...
    def parse_vicon_msgs(self, bag):
        ...
        for topic in vicon_topics:
            ...
            if name == THING_BASE_NAME:
                ts, qs = ros_utils.parse_ridgeback_vicon_msgs(
                    msgs
                )  # q is a 3-element vector x, y, theta
                self.data["vicon"]["ThingBase"] = {
                    "ts": ts,
                    "pos": qs[:, :2],
                    "orn": qs[:, 2],
                }
                ...
                ts, qs = ros_utils.parse_transform_stamped_msgs(
                    msgs, False
                )  # q is a 7-element vector x, y, z for position followed by a unit quaternion
                self.data["vicon"][name] = {
                    "ts": ts,
                    "pos": qs[:, :3],
                    "orn": qs[:, 3:],
                }
    ...
    def compute_values_from_robot_model(self):
        ...
        self.data["model"]["EE"] = {
            "ts": self.data["ur10"]["joint_states_interpolated"]["ts"].copy(),
    ...
    def plot_base_odom_velocity(self):
        ...
        axes[0].plot(
            self.data["ridgeback"]["odom"]["ts"],
            self.data["ridgeback"]["odom"]["vs"][:, 0],
            label="vx",
        )
        axes[0].plot(
            self.data["ridgeback"]["odom"]["ts"],
            self.data["ridgeback"]["odom"]["vs"][:, 1],
            label="vy",
        )
        axes[0].plot(
            self.data["ridgeback"]["odom"]["ts"],
            self.data["ridgeback"]["odom"]["vs"][:, 2],
            label="vz",
        )
        ...
        axes[1].plot(
            self.data["ridgeback"]["odom"]["ts"],
            self.data["ridgeback"]["odom"]["omegas"][:, 0],
            label="wx",
        )
        axes[1].plot(
            self.data["ridgeback"]["odom"]["ts"],
            self.data["ridgeback"]["odom"]["omegas"][:, 1],
            label="wy",
        )
        axes[1].plot(
            self.data["ridgeback"]["odom"]["ts"],
            self.data["ridgeback"]["odom"]["omegas"][:, 2],
            label="wz",
        )
    ...
    def plot_base_velocity_estimation_odom(self):
        ...
        axes[0].plot(
            self.data["ridgeback"]["joint_states"]["ts"],
            self.data["ridgeback"]["joint_states"]["vbs"][:, 0],
            "b",
            label="v_x_est",
        )
        axes[1].plot(
            self.data["ridgeback"]["joint_states"]["ts"],
            self.data["ridgeback"]["joint_states"]["vbs"][:, 1],
            "b",
            label="v_y_est",
        )
        axes[2].plot(
            self.data["ridgeback"]["joint_states"]["ts"],
            self.data["ridgeback"]["joint_states"]["vbs"][:, 2],
            "b",
            label="ω_est",
        )
    ...
    def plot_base_velocity_estimation_vicon(self, axes=None, legend=""):
        ...
        axes[0].plot(
            self.data["vicon"]["ThingBase"]["ts"][:-1],
            self.data["vicon"]["ThingBase"]["pos_diff"][:, 0],
            "r",
            label="v_x_vicon",
        )
        axes[1].plot(
            self.data["vicon"]["ThingBase"]["ts"][:-1],
            self.data["vicon"]["ThingBase"]["pos_diff"][:, 1],
            "r",
            label="v_y_vicon",
        )
        axes[2].plot(
            self.data["vicon"]["ThingBase"]["ts"][:-1],
            self.data["vicon"]["ThingBase"]["yaw_diff"][:],
            "r",
            label="ω_vicon",
        )
        ...
        axes[0].plot(
            self.data["ridgeback"]["joint_states"]["ts"],
            self.data["ridgeback"]["joint_states"]["vs"][:, 0],
            "b",
            label="v_x_est",
        )
        axes[1].plot(
            self.data["ridgeback"]["joint_states"]["ts"],
            self.data["ridgeback"]["joint_states"]["vs"][:, 1],
            "b",
            label="v_y_est",
        )
        axes[2].plot(
            self.data["ridgeback"]["joint_states"]["ts"],
            self.data["ridgeback"]["joint_states"]["vs"][:, 2],
            "b",
            label="ω_est",
        )
    ...
    def plot_base_velocity_estimation_tf(self, axes=None, legend=""):
        ...
        axes[0].plot(
            self.data["tf"]["base_link"]["ts"][:-1],
            self.data["tf"]["base_link"]["pos_diff"][:, 0],
            "r",
            label="v_x_tf",
        )
        axes[1].plot(
            self.data["tf"]["base_link"]["ts"][:-1],
            self.data["tf"]["base_link"]["pos_diff"][:, 1],
            "r",
            label="v_y_tf",
        )
        axes[2].plot(
            self.data["tf"]["base_link"]["ts"][:-1],
            self.data["tf"]["base_link"]["rpy_diff"][:, 2],
            "r",
            label="ω_tf",
        )
    ...
    def plot_base_state_estimation_vicon(self, axes=None, legend=""):
        ...
        axes[0].plot(
            self.data["vicon"]["ThingBase"]["ts"],
            self.data["vicon"]["ThingBase"]["pos"][:, 0]
            - self.data["vicon"]["ThingBase"]["pos"][0, 0]
            + self.data["ridgeback"]["joint_states"]["qs"][0, 0],
            "r",
            label="x_vicon",
        )
        axes[1].plot(
            self.data["vicon"]["ThingBase"]["ts"],
            self.data["vicon"]["ThingBase"]["pos"][:, 1]
            - self.data["vicon"]["ThingBase"]["pos"][0, 1]
            + self.data["ridgeback"]["joint_states"]["qs"][0, 1],
            "r",
            label="y_vicon",
        )
        axes[2].plot(
            self.data["vicon"]["ThingBase"]["ts"],
            self.data["vicon"]["ThingBase"]["orn"]
            - self.data["vicon"]["ThingBase"]["orn"][0]
            + self.data["ridgeback"]["joint_states"]["qs"][0, 2],
            "r",
            label="θ_vicon",
        )
        ...
        axes[0].plot(
            self.data["ridgeback"]["joint_states"]["ts"],
            self.data["ridgeback"]["joint_states"]["qs"][:, 0],
            "b",
            label="x_est",
        )
        axes[1].plot(
            self.data["ridgeback"]["joint_states"]["ts"],
            self.data["ridgeback"]["joint_states"]["qs"][:, 1],
            "b",
            label="y_est",
        )
        axes[2].plot(
            self.data["ridgeback"]["joint_states"]["ts"],
            self.data["ridgeback"]["joint_states"]["qs"][:, 2],
            "b",
            label="θ_est",
        )
    ...
    def plot_base_state_estimation_tf(self, axes=None, legend=""):
        ...
        axes[0].plot(
            self.data["tf"]["base_link"]["ts"],
            self.data["tf"]["base_link"]["pos"][:, 0],
            "r",
            label="x_tf",
        )
        axes[1].plot(
            self.data["tf"]["base_link"]["ts"],
            self.data["tf"]["base_link"]["pos"][:, 1],
            "r",
            label="y_tf",
        )
        axes[2].plot(
            self.data["tf"]["base_link"]["ts"],
            self.data["tf"]["base_link"]["rpy"][:, 2],
            "r",
            label="θ_tf",
        )
    ...
    def plot_joint_states(self, axes=None, legend=""):
        ...
        ax.plot(
            self.data["ridgeback"]["joint_states"]["ts"],
            self.data["ridgeback"]["joint_states"]["qs"],
            "-",
            label=["x", "y", "θ"],
        )
        ...
        ax.plot(
            self.data["ridgeback"]["joint_states"]["ts"],
            self.data["ridgeback"]["joint_states"]["vs"],
            "-",
            label=[r"$v_x$", r"$v_y$", r"$\omega$"],
        )
        ...
        ax = axes[2]
        ax.plot(
            self.data["ur10"]["joint_states"]["ts"],
            self.data["ur10"]["joint_states"]["qs"],
            "-",
            label=[r"$\theta_{}$".format(i + 1) for i in range(6)],
        )
        ...
        ax.plot(
            self.data["ur10"]["joint_states"]["ts"],
            self.data["ur10"]["joint_states"]["vs"],
            "-",
            label=[r"$\dot\theta_{}$".format(i + 1) for i in range(6)],
        )
        ...
        ax.plot(
            self.data["ur10"]["joint_states"]["ts"],
            self.data["ur10"]["joint_states"]["effs"],
            "-",
            label=[r"$eff_{}$".format(i + 1) for i in range(6)],
        )
    ...
    def plot_joint_vel_tracking(self, axes=None, legend=""):
        ...
        for i in range(3):
            ...
            ax[i].plot(
                self.data["ridgeback"]["cmd_vels"]["ts"],
        ...
        for i in range(6):
            ax3[i].plot(
                self.data["ur10"]["joint_states"]["ts"],
                self.data["ur10"]["joint_states"]["effs"][:, i],
                "-",
                label=[r"$eff_{}$".format(i + 1)],
            )
        ...
        if plot_ee_tracking:
            ...
            for i in range(3):
                ax.plot(
                    ...
                    rees[:, i],
                    "--",
                    label=labels[i] + "d" + subscript,
                    color=colors[i],
                )
                ax.plot(
                    self.data["vicon"][VICON_TOOL_NAME]["ts"],
                    self.data["vicon"][VICON_TOOL_NAME]["pos"][:, i],
                    "-",
                    label=labels[i] + subscript,
                    color=colors[i],
                )
            ax.set_xlabel("Time (s)")
    ...
    def plot_model_vs_groundtruth(self):
        ...
        for i in range(3):
            plt.plot(
                self.data["model"]["EE"]["ts"],
                self.data["model"]["EE"]["pos"][:, i],
                label=pos_label[i] + "_model",
                color=colors[i],
            )
            plt.plot(
                self.data["vicon"][VICON_TOOL_NAME]["ts"],
        ...
        for i in range(4):
            plt.plot(
                self.data["model"]["EE"]["ts"],
                self.data["model"]["EE"]["orn"][:, i],
                label=orn_label[i] + "_model",
                color=colors[i],
            )
            plt.plot(
                self.data["vicon"][VICON_TOOL_NAME]["ts"],
                self.data["vicon"][VICON_TOOL_NAME]["orn"][:, i],
                "--",
                label=orn_label[i] + "_meas",
                color=colors[i],
            )
...
class HTMPCDataPlotter:
    ...
    def __init__(self, folder_path, data_plotter_class=DataPlotter):
        plotter_template = construct_logger(
            folder_path, process=False, data_plotter_class=data_plotter_class
        )
    ...
    def plot_solver_status(self, axes=None, index=0, block=True, legend=None):
        ...
        for task_id, plotter in enumerate(self.plotters):
            plotter.plot_solver_status_htmpc(
                [axes[task_id]],
                block=False or (block and task_id == self.task_num - 1),
                legend=" ".join(["Task", str(task_id)]),
            )
    ...
    def plot_solver_iters(self, axes=None, index=0, block=True, legend=None):
        ...
        for task_id, plotter in enumerate(self.plotters):
            plotter.plot_solver_iters_htmpc(
                [axes[task_id]],
                block=False or (block and task_id == self.task_num - 1),
                legend=" ".join(["Task", str(task_id)]),
            )
    ...
    def plot_mpc_prediction(self, data_name, task_id=1, t=0, block=True):
        self.plotters[task_id].plot_mpc_prediction(
            data_name,
            t,
            block=block,
            legend=" ".join(["Task", str(task_id)]),
            p_struct=self.controller.stmpc_p_structs[task_id],
        )
    ...
    def plot_mpc(self):
        ...
        self.plot_mpc_prediction(
            "mpc_sdf_constraint_predictions", task_id=0, t=0.1, block=False
        )
        self.plot_mpc_prediction(
            "mpc_sdf_constraint_predictions", task_id=1, t=0.1, block=False
        )

        self.plot_mpc_prediction(
            "mpc_self_constraint_predictions", task_id=0, block=False
        )
        for task_id, stmpc_constraints in enumerate(self.controller.stmpc_constraints):
            for constraint in stmpc_constraints:
                if "Lex" in constraint.name:
                    self.plot_mpc_prediction(
                        "_".join(["mpc", constraint.name, "constraint", "predictions"]),
                        task_id=task_id,
                        block=False,
                    )
        ...
        # self.plot_mpc_prediction("mpc_control_constraint_predictions",block=False)
        # self.plot_mpc_prediction("mpc_state_constraint_predictions", block=False)
        self.plot_mpc_prediction("mpc_x_bars", task_id=0, t=0.0, block=False)
    ...
    def print_mpc_param(self, param_name):
        for task_id, plotter in enumerate(self.plotters):
            ...
            plotter.print_mpc_param(
                param_name, self.controller.stmpc_p_structs[task_id]
            )
    ...
    def plot_base_state_separate(
        ...
        axes = self.plotters[-1].plot_base_state_separate(
            axes, index, legend, linewidth, color
        )
        return axes
    ...
    def plot_base_ref_separate(
        ...
        axes = self.plotters[-1].plot_base_ref_separate(
            axes, index, legend, linewidth, color
        )
        return axes
    ...
    def plot_sdf_collision_separate(
        ...
        axes = self.plotters[-1].plot_sdf_collision_separate(
            axes, index, color, linewidth, block, legend
        )
        return axes
    ...
    def plot_base_path(
        ...
        axes = self.plotters[-1].plot_base_path(
            axes, index, legend, worldframe, linewidth, color
        )
        return axes
    ...
    def plot_base_ref_path(
        ...
        axes = self.plotters[-1].plot_base_ref_path(
            axes, index, legend, worldframe, color
        )
        return axes
    ...
    def plot_exp(self, time):
        ...
        self.plot_mpc_prediction("mpc_x_bars", task_id=1, t=0.0, block=False)
...
def construct_logger(
    ...
    ht_data_plotter_class=HTMPCDataPlotter,
):
    """Path to data folder
    ...
    if is_htmpc:
        ...
        if folder_num == 2:
            ...
            return data_plotter_class.from_PYSIM_results(
                os.path.join(path_to_folder, items[0]), process
            )
