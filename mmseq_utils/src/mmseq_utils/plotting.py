...
def construct_logger(path_to_folder, process=True):
    """ Path to data folder
    ...
    if folder_num == 2:
        ...
        return DataPlotter.from_ROSSIM_results(path_to_folder, process)
    elif folder_num == 1 and file_num == 0:
        # if generated by running controller in loop with simulator,
        # there is only one folder
        return DataPlotter.from_PYSIM_results(os.path.join(path_to_folder, items[0]), process)
    elif folder_num == 1 and file_num == 1:
        return DataPlotter.from_ROSEXP_results(path_to_folder, process)
