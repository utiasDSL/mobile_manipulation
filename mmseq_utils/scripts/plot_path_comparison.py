...
def path_comparison(folder_path):
    ...
    # plot base ref path
    plotters[-1].plot_base_ref_path(axes, num_plotters, legend="", worldframe=False, color=cm(cm_index[1]))
    ...
    # plot EE targets
    axes.scatter(EE_POS_Target0[0],EE_POS_Target0[1],  edgecolor=YELLOW, facecolor=YELLOW)
    axes.scatter(EE_POS_Target1[0],EE_POS_Target1[1], color=YELLOW)
    axes.text(EE_POS_Target0[0]+0.35, EE_POS_Target0[1], "EE \nWaypoint \n#1", horizontalalignment='center')
    axes.text(EE_POS_Target1[0]-0.25, EE_POS_Target1[1]+0.1, "EE Waypoint \n#2", horizontalalignment='center')

    #
    circle1 = plt.Circle(r_ew_w_ds[0][:2], Rc, facecolor=[0,0,0,0.015], edgecolor=cm(cm_index[0]))
    axes.add_patch(circle1)

    circle_text_phi = 0.05 * np.pi
    circle_text_pos = EE_POS_Target0 + Rc * np.array([np.cos(circle_text_phi), np.sin(circle_text_phi) ])*1.8
    ...
    axes.arrow(approach_pose[0], approach_pose[1],
               arrow_length*np.cos(approach_pose[2]), arrow_length*np.sin(approach_pose[2]), width=0.01, facecolor=SCARLET, edgecolor=SCARLET)
    closet_approaching_pose_text_pos = approach_pose[:2] + np.array([np.cos(approach_pose[2]), np.sin(approach_pose[2])]) * arrow_length * 1.75
