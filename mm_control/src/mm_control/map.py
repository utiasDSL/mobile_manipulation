...
class SDF2D:
    def __init__(self, config, init_pose=np.eye(4)):
        ...
        self.offset = (
            0.0  # safety offset. should be kept the same as line 248 in ptcloud_vis.h
        )
        self.mul = 10.0
    ...
    def create_map(self, tsdf, tsdf_vals):
        ...
        self.map = LinearNDInterpolator(
            pts, vs
        )  # choose LinearNDInterpolator(pts, vs) or CloughTocher2DInterpolator(pts, vs)
        self.map((0, 0))
    ...
    def vis(self, x_lim, y_lim, block=True):
        ...
        _, ax = plt.subplots()
        levels = np.linspace(-0.5, 1.5, int(2.0 / 0.25) + 1)
    ...
    def query_val_nonblocking(self, x, y):
        ...
        if self.valid:
            ...
            queried_pos = np.vstack(
                (x, y, np.zeros(self.get_len(x)), np.ones(self.get_len(x)))
            )
    ...
    def query_grad(self, x, y):
        ...
        self.query_val_nonblocking(x, y)
...
class SDF3D:
    ...
    def create_map0(self, tsdf, tsdf_vals):
        pts = np.around(np.array([np.array([p.x, p.y, p.z]) for p in tsdf]), 2).reshape(
            (len(tsdf), 3)
        )
        vs = [c.r * self.mul for c in tsdf_vals]
        self.mutex.acquire(blocking=True)
    ...
    def create_map(self, tsdf, tsdf_vals):
        ...
        vs = [c.r * self.mul for c in tsdf_vals]
        self.map_ir = LinearNDInterpolator(
            pts, vs
        )  # choose LinearNDInterpolator(pts, vs) or CloughTocher2DInterpolator(pts, vs) ort RegularGridInterpolator?
        ...
        np.nan_to_num(self.map_ir(xg, yg, zg), True, self.default_val)
    ...
    def create_map2(self, tsdf, tsdf_vals):
        ...
        max_x = np.around(
            min(max(pts[:, 0]), self.curr_robot_pose[0, 3] + self.map_size_xy), 2
        )
        min_x = np.around(
            max(min(pts[:, 0]), self.curr_robot_pose[0, 3] - self.map_size_xy), 2
        )
        max_y = np.around(
            min(max(pts[:, 1]), self.curr_robot_pose[1, 3] + self.map_size_xy), 2
        )
        min_y = np.around(
            max(min(pts[:, 1]), self.curr_robot_pose[1, 3] - self.map_size_xy), 2
        )
        max_z = max(pts[:, 2])
        min_z = min(pts[:, 2])
        ...
        self.mutex.acquire(blocking=True)
        self.map = RegularGridInterpolator(
            (xs, ys, zs), data, bounds_error=False, fill_value=self.default_val
        )
        self.map((0, 0, 0))
        self.mutex.release()
    ...
    def create_map3(self, tsdf, tsdf_vals):
        ...
        self.mutex.acquire(blocking=True)
        self.map = RegularGridInterpolator(
            (xs, ys, zs), data, method="linear", bounds_error=False, fill_value=None
        )  # extrapolate the values outside the map
        self.map((0, 0, 0))
        self.xs = xs
        self.ys = ys
        self.mutex.release()
    ...
    def query_grad(self, x, y, z):
        ...
        self.query_val_nonblocking(x, y, z)
