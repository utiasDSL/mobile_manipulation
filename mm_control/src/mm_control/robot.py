...
from spatialmath.base import q2r, r2q, rotz
...
def verify_link_transforms(robot_sim, sysMdls, link_names):
    for name in link_names:
        ...
        print(
            "pos diff:{}, rot diff{}".format(
                np.linalg.norm(pos_sim - pos_mdl), np.linalg.norm(rot_mdl - rot_sim)
            )
        )
...
def test_robot_mdl(args):
    ...
    robot.manipulability_fcn(mm.home)
    print("Testing motion model integrator")
    dt = 0.1
    a = 1.0
    N = 10
...
def check_maximum_reach(args):
    ...
    q_straight = [0, 0, 0, 0.5 * np.pi, 0.0, 0.0, 0.0, 0.5 * np.pi, 0.0]
...
def plot_signed_distance_gradient(
    c_cylinder, r_sphere, r_cylinder, x_range, y_range, grid_size
):
    ...
    signed_distance_sym = signed_distance_sphere_cylinder(
        c_sphere_sym, c_cylinder_sym, r_sphere_sym, r_cylinder_sym
    )
    ...
    signed_distance_fn = cs.Function(
        "signed_distance_fn",
        [c_sphere_sym, c_cylinder_sym, r_sphere_sym, r_cylinder_sym],
        [signed_distance_sym],
    )
    gradient_fn = cs.Function(
        "gradient_fn",
        [c_sphere_sym, c_cylinder_sym, r_sphere_sym, r_cylinder_sym],
        [gradient[:2]],
    )
    ...
    for i in range(grid_size):
        for j in range(grid_size):
            ...
            Z[i, j] = (
                signed_distance_fn(c_sphere, c_cylinder, r_sphere, r_cylinder)
                .full()
                .item()
            )
            grad = (
                gradient_fn(c_sphere, c_cylinder, r_sphere, r_cylinder).full().flatten()
            )
            grad_x[i, j] = grad[0]
            grad_y[i, j] = grad[1]
