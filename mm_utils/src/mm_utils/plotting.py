...
class DataPlotter:
    ...
    def plot_cost(self, axes=None, index=0, block=True, legend=None):
        ...
        if cost is None and cost_final is None:
            print("No cost data found in mpc_cost_iters or mpc_cost_finals")
            return
        ...
        if cost is not None:
            ...
            for task_idx in range(task_num):
                ...
                ax[task_idx].set_title("Task" + str(task_idx) + " Cost", fontsize=20)
                ax[task_idx].set_xticks(xticks)
        ...
        if cost_final is not None:
            for task_idx in range(task_num):
                ...
                ax[task_idx].set_title(
                    "Task " + str(task_idx) + " Final Cost", fontsize=20
                )
    ...
    def plot_solver_status(self, axes=None, index=0, block=True, legend=None):
        axes = self.plot_time_series_data(
            "mpc_solver_statuss", axes, index, False, legend
        )
        self.plot_time_series_data("mpc_step_sizes", axes, index, block, legend)

        return axes

    def plot_solver_iters(self, axes=None, index=0, block=True, legend=None):
        axes = self.plot_time_series_data("mpc_qp_iters", axes, index, False, legend)
        self.plot_time_series_data("mpc_sqp_iters", axes, index, block, legend)

        return axes

    def plot_time(self, axes=None, index=0, block=True, legend=None):
        for data_name in self.data.keys():
            if "mpc_time" in data_name:
                axes = self.plot_time_series_data(data_name, axes, index, False, legend)
    ...
    def plot_time_series_data(
        ...
        if HT_iter_num == task_num == ST_iter_num == 1:
            ...
            for task_idx in range(task_num):
                axes[task_idx].plot(
                    data[:, :, task_idx, :].flatten(),
                    "x-",
                    label=" ".join([legend] + data_name.split("_")[1:]),
                    linewidth=2,
                    markersize=8,
                )
                axes[task_idx].set_title("Task" + str(task_idx))
                axes[task_idx].set_xticks(xticks)
                axes[task_idx].set_xticklabels(xlables)
                axes[task_idx].grid("on")
        axes[-1].set_xlabel("Time (s)")
        axes[0].legend()
        plt.show(block=block)
    ...
    def plot_mpc(self):
        ...
        self.plot_time(block=False)
        self.plot_time_series_data("time_get_map", block=False)
    ...
    def plot_sdf_map(self, t, use_iter_snapshot=False, block=True, p_struct=None):
        ...
        if use_iter_snapshot:
            ...
            np.array(iter_snapshot["x_bar"])
            np.array(iter_snapshot["u_bar"])
        else:
            param_map_array = self.data["mpc_ocp_params"][t_index][0]
            if p_struct is None:
                param_map = reconstruct_sym_struct_map_from_array(
            ...
            self.data["mpc_x_bars"][t_index]
            self.data["mpc_u_bars"][t_index]
        sdf_map = self.model_interface.sdf_map
        sdf_map.update_map(
            param_map["x_grid_sdf"].toarray().flatten(),
            param_map["y_grid_sdf"].toarray().flatten(),
    ...
    def sdf_failure_debug(self, t):
        ...
        self.plot_time_series_data("mpc_solver_statuss", axes=[axes[1]], block=False)
        self.plot_sdf(t, use_iter_snapshot=True, block=True)

    def mpc_constraint_debug(self, t):
        self.plot_time_series_data("mpc_solver_statuss", block=False)
...
class ROSBagPlotter:
    def __init__(
        self, bag_file, config_file="$(rospack find mm_run)/config/robot/thing.yaml"
    ):
