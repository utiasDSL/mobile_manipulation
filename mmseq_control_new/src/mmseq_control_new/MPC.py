...
class MPC:
    def __init__(self, config):
        ...
        self.output_dir = Path(
            parse_ros_path({"package": "mmseq_control_new", "path": "acados_outputs"})
        )
    ...
    def evaluate_cbf_gammas(
        self,
        constraints: MPCConstraints.SignedDistanceConstraintCBF,
        x_bar,
        u_bar,
        nlp_p_map_bar,
    ):
    ...
    def evaluate_constraints_gradient(
        self, constraints: MPCConstraints.Constraint, x_bar, u_bar, nlp_p_map_bar
    ):
        ps = []
        cost_p_dict = constraints.get_p_dict()
...
if __name__ == "__main__":
    ...
    path_to_config = parse_ros_path(
        {"package": "mmseq_run", "path": "config/3d_collision_sdf.yaml"}
    )
