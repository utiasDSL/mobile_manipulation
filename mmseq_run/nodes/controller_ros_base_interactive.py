...
class EventHandler:
    def __init__(self, planners):
        self.pt = None
        # self.sim = sim
        self.planners = planners.copy()

    def onclick(self, event):
        print('button=%d, x=%d, y=%d, xdata=%f, ydata=%f' %
              (event.button, event.x, event.y, event.xdata, event.ydata))
        self.pt = [event.xdata, event.ydata]
        self.update(self.pt)

    def update(self, pt):
        self.planners[1].target_pos = np.array(pt)
        # self.sim.ghosts[1].position = pt + [0]
...
class ControllerROSNode:
    ...
    def run(self):
        ...
        plt.xlim(-5, 5)
        plt.ylim(-5, 5)
        handler = EventHandler(self.planners)
...
if __name__ == "__main__":
    rospy.init_node("controller_ros_base_interactive")
